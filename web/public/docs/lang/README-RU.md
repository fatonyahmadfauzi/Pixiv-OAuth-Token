...
### Последняя версия: v1.0.4 (29 марта 2026 г.)

**🐞 Исправлено**

- Портативный CLI/GUI: версия больше не возвращается к старой версии после обновления — `VERSION_FILE` / `CONFIG_FILE` теперь правильно сохраняются рядом с `.exe` вместо временной папки извлечения PyInstaller.
  — Замороженное обновление CLI: теперь корректно заменяет фактический `.exe` с помощью сценария обновления `.bat` вместо перезаписи временного файла.

**✨ Добавлено**

- **Обновление с учетом архитектуры** — интерфейс командной строки и графический интерфейс теперь автоматически определяют используемую архитектуру (`x64`, `x86`, `ARM64`) и загружают соответствующий вариант сборки при обновлении.
- **Последовательность обновления установки CLI** — CLI теперь автоматически запускает установщик установки при обновлении с установки `Program Files` (отражает поведение графического интерфейса).
