...
Tożsamość wersji/kompilacji aplikacji jest przechowywana w `version.json`.

- `version`: wersja semantyczna (`X.Y.Z`)
- `build_code`: unikalny identyfikator kompilacji

Domyślnym rozwiązaniem awaryjnym innym niż wydanie jest teraz `BUILD-UNKNOWN` (zamiast `REL-LOCAL`), podczas gdy zmiany w wydaniach generują kody kompilacji w stylu uniksowym za pośrednictwem `scripts/bump_version.py`:
...
> [!WAŻNE]
> Jeśli wprowadzasz zmiany w HTML, CSS lub JS, pamiętaj o uruchomieniu `cd web && node build_minify.js` przed wdrożeniem, aby automatycznie zaciemnić kod i skompresować zasoby.
