(function () {
  ...
  var pageLabelI18n = (scriptEl && scriptEl.getAttribute('data-page-i18n')) || '';
  var brandHref = (scriptEl && scriptEl.getAttribute('data-brand-href')) || '/';
  var showLangSwitcher = (scriptEl && scriptEl.getAttribute('data-lang-switcher')) === 'true';
  ...
  var NAV_LINKS = [
    { id: 'navHomepageLabel', href: '/',         i18n: 'navHomepageLabel', label: 'Homepage' },
    { id: 'navConsole',       href: '/#console',  i18n: 'navConsole',       label: 'Console'  },
    { id: 'navDownloads',     href: '/downloads', i18n: 'navDownloads',     label: 'Download' },
    { id: 'navTutorial',      href: '/tutorial',  i18n: 'navTutorial',      label: 'Tutorial' },
  ];

  // Build nav links HTML (exclude the ones listed in data-nav / activeNavs so each page hides its own link)
  var navLinksHTML = NAV_LINKS
    .filter(function (link) {
      return activeNavs.indexOf(link.id) === -1;
    })
    .map(function (link) {
      return '<a id="' + link.id + '" href="' + link.href + '" data-i18n="' + link.i18n + '">' + link.label + '</a>';
    })
    .join(' ');
  ...
  var brandLabelHTML = pageLabel
    ? '<span' + (pageLabelI18n ? ' data-i18n="' + pageLabelI18n + '"' : '') + '>' + pageLabel + '</span>'
    : '';

  var HEADER_HTML =
    '<div class="tutorial-docs-inner">' +
      '<div class="tutorial-docs-top">' +
        '<a class="tutorial-docs-brand" href="' + brandHref + '">' +
          '<strong>Pixiv OAuth</strong>' + brandLabelHTML +
        '</a>' +
        '<nav class="tutorial-docs-actions" aria-label="Main navigation">' +
          '<div class="sidebar-header">' +
            '<a class="sidebar-brand" href="' + brandHref + '"><strong>Pixiv OAuth</strong></a> ' +
